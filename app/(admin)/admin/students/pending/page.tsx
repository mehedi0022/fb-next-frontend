...
export default function PendingStudents() {
  ...
  useEffect(() => {
    if (!data?.data) return;
    const pendingSellers = data.data.filter((item) => item.status === "pending");
  ...
  const openPackageModal = async (targetSeller: Seller) => {
    ...
    setSelectedPackageId(
      getSellerPackageId(targetSeller),
    );
          ...
          previous.map((item) =>
            item.id === selectedSeller.id
              ? {
                  ...
                  sellerPackageName: pickedPackage?.name || item.sellerPackageName,
                }
              : item,
          ),
        );
    ...
    <div>
      ...
      <AppModal
        ...
        <div className="space-y-4">
          ...
          {initialPaymentDone && selectedSeller && getSellerPackageId(selectedSeller) && (
